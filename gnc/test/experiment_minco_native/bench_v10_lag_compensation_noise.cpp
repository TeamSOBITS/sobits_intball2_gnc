// docs/2026-09-01_replanning_minco_v4_open_issues.md 課題1・対策案3の
// ノイズ耐性検証（bench_v10）。bench_v8_root_causeの追試10・11で、
// 有限差分+EMAの合成遅れτ=dt/2+(1-alpha)/alpha*dtを解析的に見積もり
// 推定速度自体の変化率でτ分リード補正する対策（LAG_COMPENSATED）が
// ノイズなし条件では全シナリオで有効と確認できた。しかしその補正は
// EMA出力をさらに微分するため、EMAが本来抑えるはずのノイズを
// 再度増幅する可能性がある。bench_v6_noisy_feedback.cppのノイズ注入
// （観測位置にガウスノイズ、真の位置系列は汚染しない）をベースに、
// 無補正(FINITE_DIFF_EMA)と補正あり(LAG_COMPENSATED)を同一ノイズ
// シード・同一シナリオで比較する。
//
// ビルド: g++ -std=c++17 -O3 -DNDEBUG \
//   -I<repo>/minco_native_py/third_party/gcopter/gcopter/include \
//   -I/usr/include/eigen3 \
//   bench_v10_lag_compensation_noise.cpp -o bench_v10_lag_compensation_noise
// 実行: ./bench_v10_lag_compensation_noise wrench_envelope_reduced_m24_margin1.csv \
...
int main(int argc, char **argv)
{
    ...
    std::cout << "=== v10 lag-compensation noise robustness (global period=" << global_period
              << "s, t_local=" << t_local << "s, n_ticks=" << n_ticks << ", dt=" << dt
              << "s, ema_alpha=" << ema_alpha << ") ===\n";
    for (const auto &sc : scenarios)
    {
        for (double vy : initVySweep)
        {
            ...
            {
                // 同一シード・同一ノイズ系列で無補正/案3を直接比較する。
                runScenario(sc, vy, n_ticks, dt, global_period, t_local, sigma, ema_alpha, 42,
                            EstimatorMode::UNCOMPENSATED, 0.0, "UNCOMPENSATED");
                runScenario(sc, vy, n_ticks, dt, global_period, t_local, sigma, ema_alpha, 42,
                            EstimatorMode::LAG_COMPENSATED, 0.0, "LAG_COMPENSATED");
            }
        }
    }

    // 対策案4（MODEL_KF）: 指令加速度による予測+ノイズ入り位置観測での補正。
    // q_accel_std=0.01 m/s^2をデフォルトとして、同じ4シナリオ×2巡航速度×
    // 4ノイズ水準で全数比較する。
    const double q_accel_std_default = 0.01;
    std::cout << "\n=== v10 MODEL_KF (q_accel_std=" << q_accel_std_default << ") ===\n";
    for (const auto &sc : scenarios)
    {
        for (double vy : initVySweep)
        {
            ...
            {
                runScenario(sc, vy, n_ticks, dt, global_period, t_local, sigma, ema_alpha, 42,
                            EstimatorMode::MODEL_KF, q_accel_std_default, "MODEL_KF");
            }
        }
    }

    // q_accel_stdのチューニング感度: 最もシビアなケース（鋭角ターン、巡航
    // スタート、最悪ノイズ）でq_accel_stdを振ってみる。
    std::cout << "\n=== v10 MODEL_KF q_accel_std sensitivity"
                 " (sharp_turn_single_via, vy=0.3, noise_sigma=0.01) ===\n";
    for (double q : {0.0001, 0.001, 0.01, 0.1, 1.0})
    {
        runScenario(scenarios[3], 0.3, n_ticks, dt, global_period, t_local, 0.01, ema_alpha, 42,
                    EstimatorMode::MODEL_KF, q, "MODEL_KF q_accel_std=" + std::to_string(q));
    }
