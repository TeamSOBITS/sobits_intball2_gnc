                  ...
                  double ema_alpha, unsigned rng_seed)
{
    ...
    // 書き戻さない）。
    // 修正（追試8由来のバグ修正）: ループ開始前に「1回目の観測」を手動で
    // 取ってしまうと、tick 0で「前回観測」と「今回観測」が実質同一時刻の
    // ものになり、有限差分速度が見かけ上ゼロと計算されてEMAに混入する
    // （追試6-1で特定した初期化バグ）。これを避けるため、最初の観測は
    // ループ内のtick 0で行い、そのtickでは差分を取らず（真の初速度を
    // そのまま使う）、tick 1以降から実際の有限差分+EMA更新を開始する。
    Vector3d observedPosPrev;
    bool observedInit = false;
    ...
    for (int tick = 0; tick < n_ticks; tick++)
    {
        ...
        {
            observedInit = true;  // tick 0: 差分を取らず真の初速度のままにする
        }
