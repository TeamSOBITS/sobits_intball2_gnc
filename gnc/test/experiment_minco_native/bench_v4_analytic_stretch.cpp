...
int main(int argc, char **argv)
{
    ...
    auto solveGlobal = [&](const Vector3d &headPosVec, const Vector3d &headVelVec,
        ...
        for (int iter = 0; iter <= MAX_STRETCH_ITERS; iter++)
        {
            ...
            for (int i = 0; i < K; i++)
            {
                ...
                {
                    feasible = false;
                    // Fast-Planner reallocateTimeのratio=sqrt(max_acc/limit_acc)+eps
                    // 相当（wrenchは~1/T^2でスケールするため平方根）、10%キャップ。
    ...
    std::cout << "=== v4 (via retirement + analytic time-adjustment + whole-route heuristic T) hierarchical"
