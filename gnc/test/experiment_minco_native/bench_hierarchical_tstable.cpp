// bench_hierarchical_local_horizon.cppの発展版。
// docs/2026-09-01_drone_planner_global_layer_literature_review.md の調査で
// 「MINCO自体はTを完全自由変数として毎回同時最適化する構造であり、
// ドローン系プランナーはFast-Planner的フロントエンド（ヒューリスティック
// 初期時間＋動力学違反区間だけの局所調整）でTの暴れを抑えている」と
// 分かったことを受け、MINCO側のコスト関数に
// 「前回グローバルreplanのTからの乖離ペナルティ w_stability*||T-T_prev||^2」
// を追加し、グローバル層のduration jump（bench_hierarchical_local_horizonで
// z暴走・リミットサイクルの原因と特定）が抑えられるかを検証する。
//
// 比較対象:
//   - hierarchical (w_stability=0): 前回と同一ロジック（比較用）
//   - hierarchical (w_stability>0): Tの乖離にペナルティを課した版
//
// ビルド: g++ -std=c++17 -O3 -DNDEBUG \
//   -I<repo>/minco_native_py/third_party/gcopter/gcopter/include \
//   -I/usr/include/eigen3 \
//   bench_hierarchical_tstable.cpp -o bench_hierarchical_tstable
// 実行: ./bench_hierarchical_tstable wrench_envelope_reduced_m24_margin1.csv \
//        [n_ticks=900] [dt=0.1] [init_vy=0.0] [global_replan_period_s=2.0] \
//        [t_local_s=1.0] [w_stability=1e4]
...
struct EvalContext
{
    ...
    Matrix3Xd rotVia;
    // 前回グローバルreplanのT。サイズ0なら初回（ペナルティなし）。
    VectorXd Tprev;
    double wStability;
};
...
double evaluate(void *instance, const VectorXd &x, VectorXd &g)
{
    ...
    // 前回グローバルreplanのTからの乖離ペナルティ（ドローン文献裏取りで
    // 判明した「ヒューリスティック初期値+変化量抑制」を模した項）。
    double stabilityCost = 0.0;
    if (ctx->Tprev.size() == K && ctx->wStability > 0.0)
    {
        const VectorXd dT = T - ctx->Tprev;
        stabilityCost = ctx->wStability * dT.squaredNorm();
        gdT_total += (2.0 * ctx->wStability) * dT;
    }
    ...
    return W_TIME * T.sum() + W_ENERGY * (energyPos + energyRot) + penaltyCost + stabilityCost;
}
...
int main(int argc, char **argv)
{
    if (argc < 2)
    {
        std::cerr << "usage: " << argv[0]
                   << " <envelope.csv> [n_ticks=900] [dt=0.1] [init_vy=0.0]"
                      " [global_replan_period_s=2.0] [t_local_s=1.0] [w_stability=1e4]\n";
        return 1;
    }
    loadWrenchEnvelope(argv[1]);
    const int n_ticks = argc >= 3 ? std::atoi(argv[2]) : 900;
    ...
    const double w_stability = argc >= 8 ? std::atof(argv[7]) : 1e4;
    ...
    VectorXd Tprev;  // 空 = まだグローバルreplan未実施
    ...
    auto solveGlobal = [&](const Vector3d &headPosVec, const Vector3d &headVelVec,
        ...
        ctx.rotVia = rotVia;
        ctx.Tprev = Tprev;
        ctx.wStability = w_stability;

        VectorXd x0 = VectorXd::Zero(3 * numVia + K);
        VectorXd T0 = Tprev.size() == K ? Tprev : VectorXd::Constant(K, INITIAL_SEGMENT_TIME);
        VectorXd tau0;
        backwardT(T0, tau0);
    ...
    std::cout << "=== hierarchical+Tstability (global period=" << global_period << "s, t_local=" << t_local
              << "s, w_stability=" << w_stability << ") (K=" << K << ", n_ticks=" << n_ticks
              << ", dt=" << dt << "s) ===\n";
