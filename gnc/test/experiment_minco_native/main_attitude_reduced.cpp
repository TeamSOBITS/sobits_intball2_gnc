// Variant of main_attitude.cpp for testing speed-improvement option (3) from
// docs/2026-08-29_minco_attitude_torque_integration_plan.md ("面数削減 --
// 内接近似"): the optimizer's penalty function is driven by a REDUCED-facet
// envelope (F_ENV/G_ENV, loaded from argv[1]) built as the convex hull of a
// subset of the true envelope's 256 exact vertices (see
// scratchpad/gen_reduced_envelopes.py) -- guaranteed inscribed (a subset of
// the true vertex set) by construction, not a heuristic approximation. The
// resulting trajectory is then checked for real safety against the TRUE
// (full, 9764-facet) envelope loaded separately as F_TRUE/G_TRUE, since
// "feasible w.r.t. the reduced envelope" doesn't by itself prove "feasible
// w.r.t. the true envelope" -- inscribed-by-construction guarantees the
// reduced envelope's *feasible region* is a subset of the true one, but the
// optimizer only enforces the reduced constraint during the solve, so this
// still needs an explicit post-hoc check against F_TRUE/G_TRUE.
//
...
double maxViolation(minco::MINCO_S3NU &posMinco, minco::MINCO_S3NU &rotMinco, const VectorXd &T,
                     const MatrixXd &F, const VectorXd &G)
{
                   ...
                   const Vector3d &q1Given, const Vector3d &rv1, double q1HalfWidth)
{
    ...
    result.T1 = T(0);
    result.T2 = T(1);
    result.maxViolReduced = maxViolation(posMinco, rotMinco, T, F_ENV, G_ENV);
    result.maxViolTrue = maxViolation(posMinco, rotMinco, T, F_TRUE, G_TRUE);
...
int main(int argc, char **argv)
{
    if (argc < 2)
    {
        std::cerr << "usage: " << argv[0]
                   << " <reduced_envelope.csv> [true_envelope.csv] [scenario.txt]\n";
        return 1;
    }
    const std::string truePath = argc >= 3 ? argv[2] : "wrench_envelope.csv";
    loadWrenchEnvelope(argv[1], F_ENV, G_ENV);
    loadWrenchEnvelope(truePath, F_TRUE, G_TRUE);
    std::cout << "reduced (optimization) envelope: " << F_ENV.rows() << " halfspaces\n";
    std::cout << "true (verification) envelope: " << F_TRUE.rows() << " halfspaces\n";

    Vector3d Q0 = NEAR_DOCK;
    Vector3d Q2 = ABOVE_DOCK;
    Vector3d q1Given;
    Vector3d rv1;
    if (argc >= 4)
    {
        // scenario file (see gen_scenario.py): Q0, Q2, q1_given, rv1 -- one
        // 3-vector per line, generated from (leg1_len, leg2_len, turn_deg)
        // since MASS/INERTIA are isotropic scalars, only leg lengths and turn
        // angle matter for the dynamics, not absolute position/orientation.
        std::ifstream fp(argv[3]);
        if (!fp)
        {
            throw std::runtime_error("cannot open scenario file " + std::string(argv[3]));
        }
        fp >> Q0(0) >> Q0(1) >> Q0(2);
        fp >> Q2(0) >> Q2(1) >> Q2(2);
        fp >> q1Given(0) >> q1Given(1) >> q1Given(2);
        fp >> rv1(0) >> rv1(1) >> rv1(2);
    }
    else
    {
        const Vector3d midpoint = 0.5 * (Q0 + Q2);
        const Vector3d bulge = NAV_ENTRY - midpoint;
        q1Given = midpoint + BULGE_SCALE * bulge;
        rv1 = RV1;
    }
    ...
    std::cout << "scenario: Q0->q1_given->Q2, turn_angle=" << turnAngleDeg << "deg\n\n";
                        ...
                        std::make_pair("minco-style (q1 free +-0.3m)", 0.3)})
    {
        ...
        std::cout << "T1, T2: " << r.T1 << ", " << r.T2 << "  (total " << (r.T1 + r.T2)
                  << " s)\n";
        std::cout << "max violation (reduced envelope): " << r.maxViolReduced << "\n";
