                  ...
                  double ema_alpha, unsigned rng_seed)
{
    ...
    for (int tick = 0; tick < n_ticks; tick++)
    {
        ...
        if (tick % 5 == 0)
        {
            std::cout << "  t=" << (tick * dt) << " headPos=(" << headPosTrue.transpose()
                       << ") headVel=(" << headVelTrue.transpose() << ") vEst=(" << vEst.transpose()
                       << ") tgtPos=(" << tgtPos.transpose() << ") tgtVel=(" << tgtVel.transpose()
                       << ") tgtAcc=(" << tgtAcc.transpose() << ") globalDur=" << globalT.sum() << "\n";
        }
    }
...
int main(int argc, char **argv)
{
    ...
    std::cout << "=== v6 DEBUG single-case (global period=" << global_period << "s, t_local=" << t_local
              << "s, n_ticks=" << n_ticks << ", dt=" << dt << "s, ema_alpha=" << ema_alpha << ") ===\n";
    runScenario(sc, 0.3, n_ticks, dt, global_period, t_local, 0.0, ema_alpha, 42);
