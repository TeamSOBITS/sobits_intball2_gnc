// bench_v7_attitude_and_k.cpp（姿勢非trivial・K>2への汎化検証、ベース設計
// は真の速度を境界条件に使う）に、bench_v10/v11で検証したノイズ入り
// 速度推定+MODEL_KF（対策案4）を組み込んだもの。姿勢非trivial・多経由点
// という条件でも対策案4が有効かを検証する（オフライン検証の残項目1）。
//
// 姿勢(rotMinco)側は元のv7のまま（headRotは"q0基準"の相対rotvecでゼロ
// 固定、姿勢の角速度推定は本検証の対象外）。位置(posMinco)側の速度
// 境界条件だけ、真の値の代わりにノイズ入り観測+MODEL_KF推定を使う。
//
// ビルド: g++ -std=c++17 -O3 -DNDEBUG \
//   -I<repo>/minco_native_py/third_party/gcopter/gcopter/include \
//   -I/usr/include/eigen3 \
//   bench_v12_attitude_k_kf.cpp -o bench_v12_attitude_k_kf
// 実行: ./bench_v12_attitude_k_kf wrench_envelope_reduced_m24_margin1.csv \
...
void runScenario(const Scenario &scenario, double initVy, int n_ticks, double dt,
                  ...
                  double q_accel_std, unsigned rng_seed)
{
    ...
    // 対策案4（MODEL_KF）: bench_v10/v11と同じ、指令加速度による予測+
    // ノイズ入り位置観測での補正（定加速度モデルの簡易カルマンフィルタ）。
    ...
    int globalK;
    solveGlobal(headPos, kfVel, route, rotRoute, globalT, globalCoeffsPos, globalK);
    ...
    {
        // MODEL_KF予測+補正（bench_v10/v11と同一のロジック）。
        const Vector3d observedPos = headPos + noiseVec();
        ...
        {
            solveGlobal(headPos, vUsed, route, rotRoute, globalT, globalCoeffsPos, globalK);
            globalElapsed = 0.0;
        }
    ...
    std::cout << "[" << scenario.name << " initVy=" << initVy << " noise_sigma=" << position_noise_sigma
               << "] FINAL=(" << headPos.transpose() << ")  target=(" << target.transpose()
               << ")  err=" << err << "m  |v|=" << headVel.norm()
...
int main(int argc, char **argv)
{
    ...
    // TF実測ノイズ相当〜悲観値（bench_v10/v11と同じ水準）。
    std::vector<double> noiseSigmaSweep = {0.0, 0.001, 0.01};
    const double q_accel_std = 0.01;  // bench_v10/v11で採用済みの値

    std::cout << "=== v12 attitude+K generalization with MODEL_KF (global period=" << global_period
              << "s, t_local=" << t_local << "s, n_ticks=" << n_ticks << ", dt=" << dt
              << "s, q_accel_std=" << q_accel_std << ") ===\n";
    for (const auto &sc : scenarios)
    {
        for (double vy : initVySweep)
        {
            ...
            {
                runScenario(sc, vy, n_ticks, dt, global_period, t_local, sigma, q_accel_std, 42);
            }
        }
    }
