...
bool g_uniformStretch = false;  // true: EGO-Planner方式(全区間を同率で一律伸長)
...
void setUniformStretch(bool v) { g_uniformStretch = v; }
                                   ...
                                   double max_angular_accel = 0.0)
{
    ...
    try
    {
        ...
        for (int iter = 0; iter <= g_stretchMaxIters; iter++)
        {
            ...
            bool feasible = maxRatio.maxCoeff() <= 1.0 + VIOLATION_TOLERANCE;
            if (!feasible)
            {
                if (g_uniformStretch)
                {
                    // bench_v20診断: MINCOはセグメント間が境界条件で結合された
                    // グローバルな系なので、違反セグメントだけを個別に伸ばす
                    // (下のelse節、本番の現行方式)と、ある区間を伸ばした副作用で
                    // 別の区間が新たに違反側に押し出される「モグラ叩き」が起きる
                    // ことをverboseログで確認した。EGO-Planner
                    // (uniform_bspline.cpp lengthenTime)は個別区間ではなく全区間を
                    // 同じ比率で一律伸長する——その方式を模した比較対象。
...
int main(int argc, char **argv)
{
    ...
    if (argc >= 3)
    {
        ...
        // ---- 実際に本番へ適用するスコープの確認: 本番のplanMincoHeuristicTime
        // には回転考慮ヒューリスティック(max_angular_rate/accel)が一切ない
        // (heuristicSegmentTimesはtranslation-onlyのまま)。今回の修正対象は
        // 伸長ループのみ(uniform化 + 1.1/3→2.0/15)であり、回転floorは
        // 追加しない。translation-onlyの初期ヒューリスティック(=0,0)のまま
        // uniform stretchだけで収束するか、ここで直接確認する
        // (これまでの検証は全て回転floor込みで行っていたため未確認だった)。
        minco_native::setStretchParams(2.0, 15);
        minco_native::setUniformStretch(true);
        minco_native::setVerboseStretch(true);
        const minco_native::PlanResult rProdScope = minco_native::planMincoHeuristicTime(
            realWp, v0Real, w0Real, targetSpeedReal, maxAccelReal, viaHalfWidthReal,
            wrenchSafetyMarginReal, 0.0, 0.0);
        minco_native::setVerboseStretch(false);
        minco_native::setUniformStretch(false);
        minco_native::setStretchParams(1.1, 3);
        std::cout << "*** PRODUCTION-SCOPE CHECK: translation-only heuristic (unchanged) + "
                     "uniform stretch(2.0/15): success="
                   << rProdScope.success << " error_code=" << rProdScope.error_code
                   << " duration=" << rProdScope.duration << " ***" << std::endl;

        // ---- 診断1: コスト計測込みの最小十分パラメータ探索。
        // 「反復回数/1回あたり伸長上限を上げれば収束する」ことはbench_v18で
        // 確認済み。ここでは(a)本番デフォルトからの片側スイープで最小十分な
        // 値を探し、(b)各呼び出しの壁時計時間を計測してコスト効率を見る
        // （伸長1回ごとにevaluateFixedTのフルLBFGS再最適化が走るので、
        // 反復回数に比例してコストが増えるはず——実際にそうなるか確認）。
        std::cout << "\n--- diag1(v20): per-segment coupling mechanism trace ---" << std::endl;
        // 「非単調」の正体を掘る: via_half_width=0なので位置側のx（tanh経由）は
        // qViaに一切効かず（0倍で消える）、evaluateFixedTのLBFGSは事実上no-op。
        // つまり固定Tに対する軌道は「MINCOの閉形式最小エネルギー多項式」のみで
        // 決まり、最適化の局所解ゆらぎではない。伸長ループがTを動かすと、
        // MINCOの帯行列（隣接セグメント間の連続性条件）を通じて「今動かした
        // セグメントとは別のセグメント」の違反が変化しうる、という結合仮説を
        // 検証するため、各設定について最終反復の「どのセグメント・どのfacetが
        // 違反しているか」と最終Tベクトルを記録する。
        auto verboseRun = [&](double limitRatio, int maxIters) {
            minco_native::setStretchParams(limitRatio, maxIters);
            minco_native::setVerboseStretch(true);
            std::cerr << "\n>>> config: limit_ratio=" << limitRatio << " max_iters=" << maxIters
                       << " <<<" << std::endl;
            const minco_native::PlanResult r = minco_native::planMincoHeuristicTime(
                realWp, v0Real, w0Real, targetSpeedReal, maxAccelReal, viaHalfWidthReal,
                wrenchSafetyMarginReal, 90.0 * M_PI / 180.0, 2.4 * M_PI / 180.0);
            minco_native::setVerboseStretch(false);
            std::cout << "  stretch_limit_ratio=" << limitRatio << " stretch_max_iters=" << maxIters
                       << ": success=" << r.success << " error_code=" << r.error_code
                       << " duration=" << r.duration << " T=[";
            for (double t : r.segment_times) { std::cout << t << " "; }
            std::cout << "]" << std::endl;
            return r;
        };
        std::cout << "  [fix max_iters=10, sweep limit_ratio]" << std::endl;
        for (double ratio : {1.1, 1.2, 1.3, 1.5, 2.0, 3.0})
        {
            verboseRun(ratio, 10);
        }
        std::cout << "  [fix limit_ratio=1.1, sweep max_iters]" << std::endl;
        for (int iters : {3, 5, 8, 10, 15, 20, 30})
        {
            verboseRun(1.1, iters);
        }
        minco_native::setStretchParams(1.1, 3);

        // ---- 診断1b: EGO-Planner方式（全区間を同じ比率で一律伸長）との比較。
        // per-segment個別伸長は「あるセグメントを伸ばすとMINCOの結合を通じて
        // 別のセグメントが新たに違反側に押し出される」モグラ叩きが起きることを
        // 上のverboseログで確認した。全区間一律なら全セグメントの境界条件が
        // 同時に緩むので、この副作用が起きないはず、という仮説を検証する。
        std::cout << "\n--- diag1b: uniform (EGO-Planner-style) stretch vs per-segment ---"
                   << std::endl;
        for (auto cfg : std::vector<std::tuple<double, int, bool>>{
                 {1.1, 3, false}, {1.1, 10, false}, {2.0, 10, false}, {3.0, 10, false},
                 {1.1, 3, true}, {1.1, 10, true}, {1.2, 10, true}, {1.3, 10, true},
                 {1.4, 10, true}, {1.5, 10, true}, {1.7, 10, true}, {2.0, 10, true},
                 {3.0, 10, true}, {1.3, 20, true}, {1.2, 20, true}, {1.1, 20, true}})
        {
            const double ratio = std::get<0>(cfg);
            const int iters = std::get<1>(cfg);
            const bool uniform = std::get<2>(cfg);
            minco_native::setStretchParams(ratio, iters);
            minco_native::setUniformStretch(uniform);
            ...
            std::cout << "  limit_ratio=" << ratio << " max_iters=" << iters
                       << " uniform=" << uniform << ": success=" << r.success
        ...
        // ---- 汎化性チェック: diag1で見つけた候補パラメータが、既存の
        // 合成シナリオ（単一区間96.4度sweep、複合1〜4）を壊さないか
        // （特にrotation heuristic無効の翻訳のみbaselineケースが不変か）
        // を確認する。候補は診断結果を見て選ぶ想定だが、まずは代表的に
        // (limit_ratio=2.0, max_iters=10)を候補として試す。
        std::cout << "\n=== generalization check: synthetic scenarios under PROD stretch "
                     "params (1.1 / 3 iters) ==="
                   << std::endl;
        minco_native::setStretchParams(1.1, 3);
        runSyntheticScenarios();

        std::cout << "\n=== generalization check: synthetic scenarios under CANDIDATE stretch "
                     "params (2.0 / 10 iters, per-segment -- KNOWN NON-MONOTONIC, for reference) ==="
                   << std::endl;
        minco_native::setStretchParams(2.0, 10);
        runSyntheticScenarios();
        minco_native::setStretchParams(1.1, 3);

        // ---- 汎化性チェック本命: uniform stretch (limit_ratio=2.0, max_iters=15)
        // を合成シナリオ一式（単一区間・jump sweep・composite1〜4）+ 実24点
        // ルートの両方で、正しいwrench envelopeを使って通しで確認する。
        // ratio=2.0はdiag1bで実24点ルートに対しmax_iters=10で既に成功
        // 確認済みだが、iters=15と余裕を持たせて再確認する。壁時計コストも
        // 合成シナリオ全体でまとめて計測する。
        std::cout << "\n=== generalization check: synthetic scenarios under UNIFORM stretch "
                     "(limit_ratio=2.0, max_iters=15) ==="
                   << std::endl;
        minco_native::setStretchParams(2.0, 15);
        minco_native::setUniformStretch(true);
        {
            ...
            runSyntheticScenarios();
            const auto t1 = std::chrono::steady_clock::now();
            std::cout << "  [synthetic scenario suite total wall_ms="
                       << std::chrono::duration<double, std::milli>(t1 - t0).count() << "]"
                       << std::endl;
        }
        std::cout << "\n--- re-check: real 24pt zeno route under UNIFORM (2.0/15) ---" << std::endl;
        {
            const auto t0 = std::chrono::steady_clock::now();
            const minco_native::PlanResult r = minco_native::planMincoHeuristicTime(
                realWp, v0Real, w0Real, targetSpeedReal, maxAccelReal, viaHalfWidthReal,
                wrenchSafetyMarginReal, 90.0 * M_PI / 180.0, 2.4 * M_PI / 180.0);
            const auto t1 = std::chrono::steady_clock::now();
            std::cout << "  success=" << r.success << " error_code=" << r.error_code
                       << " duration=" << r.duration << " wall_ms="
                       << std::chrono::duration<double, std::milli>(t1 - t0).count() << std::endl;
        }
        minco_native::setUniformStretch(false);
        minco_native::setStretchParams(1.1, 3);
...
// bench_v19診断: 伸長パラメータ変更が既存の合成シナリオ（単一区間96.4度、
// 複合1〜4）を壊さないか（汎化性チェック）を、異なるstretchパラメータで
// 繰り返し実行できるよう関数化した（元のmain()末尾のコードをそのまま
// 移動しただけ、ロジック変更なし）。
void runSyntheticScenarios()
{
