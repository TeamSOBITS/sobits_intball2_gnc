...
**For "just verify this `move_to`", use `move_to_full_analysis.py` first** --
one command drives a real `move_to` goal and reports position/attitude
tracking error, fan-duty saturation, and commanded-vs-achieved wrench in a
single summary, no manual CSV inspection needed. It supersedes and replaces
the former `measure_position_tracking_error.py` and `move_to_wrench_duty_trace.py`
(both removed, 2026-08-30 -- fully covered, the latter's TF logging was even
lower-fidelity, fixed-rate instead of event-driven). `measure_attitude_tracking_error.py`
survives only for its kp_att recommendation derivation (not folded into the
new script) -- it otherwise predates `move_to` and assumes the older workflow
of running a separate trajectory-setpoint script alongside it.

| Script | Publishes to | Description |
|---|---|---|
| `move_to_full_analysis.py` | (read-only TF/setpoint/`/ctl/duty`/`/ctl/wrench`/`/ctl/wrench_achieved`, drives `/gnc/move_to`) | One-shot `move_to` verification: sends a real goal (any `guidance.trajectory_tracking_mode`, `--set-mode` to change it first) while recording everything event-driven, then prints a tracking-quality report (position/attitude error vs setpoint, fan-duty saturation, wrench desired-vs-achieved, final arrival accuracy) and writes raw per-topic CSVs (survives Ctrl-C/exception, not just clean completion): `python3 move_to_full_analysis.py nav_entry --set-mode static_minco --out-dir /tmp/trace --tag run1` |
...
| `log_replanning_attitude_trace.py` | (read-only TF + setpoint, drives `/gnc/move_to`) | Sends one or more `move_to` goals while recording every single TF sample and every single `/gnc/trajectory_setpoint` message, event-driven (no polling timer, no dropped ticks), to separate CSVs -- for the `replanning`-mode attitude-pointing-degradation investigation (`docs/archive/achieved/2026-08-25_guidance_realtime_replanning_sim_verification.md` 6 節). Tags each setpoint row with a `likely_replan_tick` flag derived purely from message order (every Nth tick since the leg started, N fetched live from `/guidance_node`'s `guidance.rate`/`replan_rate_hz` params), so re-plan-boundary jumps in `v_des`/`q_des` can be correlated against `q_act` lag without adding any instrumentation to `guidance_executor.py`: `python3 log_replanning_attitude_trace.py near_dock --return-to nav_entry --trials 5 --set-mode replanning --out-dir /tmp/replan_trace --tag run1` |
