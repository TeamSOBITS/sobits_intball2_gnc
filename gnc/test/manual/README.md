...
|---|---|---|
| `move_to_full_analysis.py` | (read-only TF/setpoint/`/gnc/checkpoints`/`/ctl/duty`/`/ctl/wrench_correction`/`/ctl/wrench_achieved`, drives `/gnc/move_to`) | One-shot `move_to` verification: sends a real goal (any `guidance.trajectory_tracking_mode`, `--set-mode` to change it first) while recording everything event-driven, then prints a tracking-quality report (position/attitude error split into trajectory phase vs setpoint and align phase vs `/gnc/checkpoints`, fan-duty saturation, wrench desired-vs-achieved, final arrival accuracy) and writes raw per-topic CSVs (survives Ctrl-C/exception, not just clean completion). `--timeout-sec` (sim time, default 90) cancels the goal if no result arrives in time: `python3 move_to_full_analysis.py nav_entry --set-mode static_minco --out-dir /tmp/trace --tag run1` |
