...
send_curve_via_naventry_to_near_dock.py (current pose -> nav_entry -> near_dock,
...
def main():
    ...
    node_name = f"send_curve_naventry_facing_{int(time.monotonic() * 1000) % 1000000}"
    node = Node(
        node_name,
    ...
    node.get_logger().info(
        ...
        f"-> p1={p1.tolist()} (near_dock) over {DURATION_SEC}s, facing direction of travel"
    )
    ...
    prev_q_des = q_align
    dt = 1.0 / RATE_HZ
    # sim clock (not time.monotonic()) so tau tracks how far simulated time
    # has actually advanced, not wall-clock time -- under RTF<1 (e.g. bridge
    # CPU contention) a wall-clock tau would race ahead of what the vehicle
    # can actually achieve in sim time.
