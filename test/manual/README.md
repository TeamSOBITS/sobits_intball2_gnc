...
| `move_to_wrench_duty_trace.py` | (read-only TF/`/ctl/duty`/`/ctl/wrench`/`/ctl/wrench_achieved`, drives `/gnc/move_to`) | Sends a real `move_to` goal (via `MoveToClient`) to a named TF location while logging TF pose (position + quat) at a configurable rate, plus `/ctl/duty`, `/ctl/wrench` (desired), and `/ctl/wrench_achieved` (event-driven) for the entire action lifetime -- for diagnosing whether a tracking failure (large attitude/position error) is caused by fan-duty/force-torque saturation rather than the reference itself: `python3 move_to_wrench_duty_trace.py near_dock --out-csv /tmp/trace.csv`. See `docs/2026-08-28_toppra_static_path_attitude_overshoot_incident.md` |
